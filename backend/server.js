...
const corsOrigin = allowedOrigins.includes("*") || allowedOrigins.length === 0
  ? true
  : allowedOrigins;
...
app.get("/", (req, res) => {
  res.json({ message: "Transmaa Backend is running" });
});
...
async function start() {
  ...
  app.listen(PORT, () => {
    console.log(`Transmaa Backend running on http://localhost:${PORT}`);
  });
}
